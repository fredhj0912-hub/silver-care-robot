...
> **상태: Part 1 · Part 2 코드 완료 (2026-09-07). 남은 것은 파이 실측뿐이다.**
>
> **남은 일은 파이 앞에서만 된다** — EC2에 프론트를 아직 재빌드하지 않았다. 파이 앞에
> 앉는 날 EC2에서 `cd frontend && npm run build` 한 번이면 켜진다. 일부러 미뤘다:
> 이 변경은 **음성 응급 경로를 없애므로**, 아무도 안 보는 사이에 먼저 배포되면 안 된다.
>
> 볼 것은 TODO.md 0순위에 적어 뒀다(버튼 크기·흐름·8초·`[QUOTA]` 로그).
>
> **다 끝나면 이 문서를 지우고** TODO.md 의 포인터 줄도 같이 지운다 — 완료 이력은 `git log` 에 남는다.
>
> ### Part 1 에서 실제로 한 것과, 계획과 달랐던 점
>
> - 계획대로: `api_usage` 테이블(스키마 2개), `repositories/usage.js`, `services/budget.js`,
>   `withRetry()` 와 `synthWithRetry()` 두 곳의 강제 지점, `GEMINI_ENABLED` 킬 스위치,
>   `GET /api/status` 의 `usage`, `[QUOTA]` 로그.
> - `tts.js` 의 함수 이름은 `isServerTtsAvailable()` 이 아니라 **`isEnabled()`** 였다.
> - **mock 으로 떨어진 이유를 `unavailableReason()` 한 곳으로 모았다** — 킬 스위치가
...
## Part 1 — 서버측 일일 예산 상한 (백엔드) ✅ 2026-09-07 완료
...
## Part 2 — 푸시투토크 (프론트) ✅ 2026-09-07 코드 완료 (파이 실측만 남음)
