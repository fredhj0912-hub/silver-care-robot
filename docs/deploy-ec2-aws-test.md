...
3. 규칙 추가 → 포트 범위 **22**(SSH) → 소스 **내 IP**(My IP, AWS 콘솔이 자동으로
   현재 IP를 채워준다) — S3 쓰기 권한이 있는 인스턴스라 SSH를 인터넷 전체에 열어두면
   안 된다. IP가 바뀌면 규칙을 다시 편집한다
4. EC2 생성 화면으로 돌아와 방금 만든 보안 그룹을 선택
...
다른 터미널에서 낙상 이벤트를 흉내 내면 스냅샷 저장 → 알림 생성까지 전체 경로가 돈다:

```bash
npm run mock-detector -- --type fall --confidence 0.92
```

이후 `GET /api/alerts`로 알림을 조회하고, 응답의 `snapshotUrl`을 열어 이미지가 실제로
서빙되는지 확인한다(S3에서 읽어 프록시로 내려준다).
