 ...
 * 웨이크워드와 그 오인식 변형.
 *
 * 노인 발음 + STT 오인식이 겹치므로 정확히 "돌봄아"로 들어올 것을 기대하면 안 된다.
 * 받침 ㅁ이 특히 잘 뭉개져 "돌보마", "돌봉아", "도봄아" 처럼 돌아온다.
 *
 * ⚠️ 맨 "돌봄"은 일부러 넣지 않았다. 이름이 곧 일상어라("돌봄 서비스가 필요해")
 * 넣는 순간 어르신이 로봇을 부르지 않았는데도 게이트가 열린다. 이름을 부르는 꼴
 * (돌봄아/돌봄이/돌보미)만 받는다. 같은 이유로 "돌봐", "도움아"도 뺐다.
 * 응급 발화는 이 목록과 무관하게 BYPASS_PHRASES 로 통과하므로 좁혀도 안전하다.
 */
// 부르는 꼴 — 문장 어디에 있어도 인식한다. 일상어와 겹치지 않는다.
const WAKE_VARIANTS = [
  '돌봄아', '돌봄야', '돌보마', '돌보매', '돌보마아',
  '도봄아', '도보마', '돌범아', '돌본아', '돌봉아',
  '돌뽐아', '돌폼아', '돌옴아', '톨봄아', '골봄아',
  '놀봄아', '홀봄아', '돌모마', '돌보나',
];
...
/**
 * 일상어와 겹치는 꼴 — **그것만 말했을 때만** 웨이크워드로 친다.
 *
 * "돌봄이"는 이름이면서 동시에 "돌봄 + 이"(주격 조사)다. 문장 안에서 찾으면
 * "돌봄이 필요한 나이지" 같은 평범한 말에 로봇이 끼어든다. "돌보미"도 마찬가지로
 * 직업을 가리키는 일상어다("돌보미 아주머니"). 그래서 이 목록은 발화 전체가
 * 그 이름일 때만 통과시킨다 — 어르신이 이름만 부르는 경우다.
 */
const WAKE_EXACT = [
  '돌봄이', '돌보미', '돌봄', '도봄이', '도보미', '돌부미',
];
...
export function containsWakeWord(text) {
  ...
  if (!t) return false;
  if (WAKE_EXACT.includes(t)) return true;
 ...
 * "돌봄아 오늘 날씨 어때" → "오늘 날씨 어때"
 * 웨이크워드만 부른 경우("돌봄아") 빈 문자열이 된다.
 */
export function stripWakeWord(text) {
  // 이름만 부른 경우 — 남길 용건이 없다
  if (WAKE_EXACT.includes(normalize(text))) return '';
