...
# **같은 글자를 다시 보내지 않는다. 주기적 갱신도 하지 않는다.**
#
# 2026-09-03에 세 가지를 실물로 시험했고 결과가 한 방향을 가리켰다:
#   한 번만 보냄  → 잘 돈다
#   200ms마다 반복 → 안 돈다
#   1000ms마다 반복 → 안 돈다
# 즉 우노가 같은 글자를 두 번 받으면 멈춘다(토글이거나 매번 재초기화하는 펌웨어).
#
# 그래서 **자가 복구를 넣을 수 없다.** 바퀴가 뭔가에 걸려 우노가 스스로 서면 파이는
# 그것을 알 방법이 없고(우노는 아무것도 보내지 않는다), 같은 글자를 다시 보내면
# 오히려 상황이 나빠진다.
#
# 복구는 사람이 한다: **손을 뗐다 다시 누르면** stop() 이 'S' 를 보내 마지막 전송이
# 초기화되고, 다음 drive() 가 방향 글자를 새로 보낸다. 실제 조작에서 자연스러운 동작이다.
# 진짜로 자동 복구를 하려면 우노 펌웨어가 상태를 되돌려 주어야 한다.

# **정지도 한 번만 보낸다.**
#
# 처음에는 "'S'는 몇 번 받아도 안 움직이니 유실에 대비해 3번 보내자"고 했다. 그 판단은
# 우노가 같은 글자를 두 번 받으면 멈춘다는 것을 알기 전의 것이었고, 그 성질은 'S'에도
# 적용된다 — 3번이면 정지·시작·정지처럼 토글돼 보드가 어떤 상태로 남는지가 매번 달라진다.
# 실제로 누를 때마다 되기도 하고 안 되기도 했다(2026-09-03).
#
# 안전 논리도 함께 뒤집힌다: 토글 펌웨어에서 정지 명령을 반복하는 것은 안전장치가 아니라
# **로봇을 다시 출발시킬 수 있는 행동**이다. 유실 대비는 펌웨어가 멱등해진 뒤에 다시 본다.
STOP_REPEATS = 1
