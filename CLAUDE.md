...
- `frontend/.env`: `VITE_ROBOT_API_KEY` (must match backend's `ROBOT_API_KEY` — ships in the client bundle, a LAN speed-bump, not real auth), `VITE_VISION_ENABLED` (default `false`, Gemini 분석까지 하는 경로 — 대화와 같은 통을 쓴다), `VITE_VISION_INTERVAL_MS`, `VITE_SNAPSHOT_ENABLED`/`VITE_SNAPSHOT_INTERVAL_MS` (분석 없이 사진만 올리는 경로 — Gemini를 안 쓴다), `VITE_VAPID_PUBLIC_KEY` (must match backend's `VAPID_PUBLIC_KEY`), `VITE_STT_MODE` (`server`|`browser`, default `server` — the Pi's Chromium cannot do Web Speech API, so `browser` is for dev machines only).
