...
- [ ] ~~Chromium 음성 인식~~ → **❌ 실패 확정.** 아래 새 1순위 항목으로 옮김
- [ ] **§8 낙상 알림 배관** — 미착수. 다음 세션
- [ ] **§9 구동부 실사** (GPIO/I2C 연결 확인) — 미착수. 다음 세션
- [ ] 원격조종 왕복 지연 측정 → **데드맨 500ms vs 폴링 2.5초** 정책 확정 (아래 B 트랙)

### 🔴 새 1순위 — 어르신이 쓸 수 있는 입력 경로가 없다 (2026-09-01)

실측 결과 **살아 있는 입력은 폰 보호자 앱 하나뿐**이다. 어르신은 폰도 키보드도 쓰지 않는다.

| 입력 | 상태 |
|---|---|
| 음성 ("효돌아") | ❌ 파이 OS Chromium이 구글 음성 키 없이 빌드됨 → 매번 `network` 오류 |
| 화면 텍스트 입력 | ⚠️ 파이에 **가상 키보드가 없다** — 물리 키보드 필요 |
| 폰 `/guardian/send` | ✅ 동작 |

- [ ] **서버측 STT** — 브라우저에서 오디오를 녹음해 백엔드로 보내고 Gemini가 받아쓰게 한다.
      `lib/stt.js`의 `createRecognizer`만 바꾸면 되도록 설계해 뒀다. arm64용 구글 크롬 정식
      빌드가 없어 브라우저 교체는 경로가 아니다. **시연 전 최우선.**
- [ ] (차선) `squeekboard` 같은 가상 키보드 설치 — 터치스크린이라야 의미가 있다
...
- [ ] **🔴 화면이 세로로 나온다 — 가로로 돌려야 한다** (09-01 발견). 키오스크는 800×480
      **가로**를 전제로 만들었다(`index.css`의 `.kiosk-root`, 얼굴 SVG 배치). 세로로 뜨면
      레이아웃이 통째로 어긋난다. **CSS로 돌리지 말 것** — 터치 좌표가 같이 안 돌아서 엉뚱한
      곳이 눌린다. **OS 출력 레벨에서 회전**시킨다:
      ```bash
      wlr-randr                                   # 출력 이름과 현재 transform 확인
      wlr-randr --output <이름> --transform 90    # 90 / 180 / 270 중 맞는 값
      ```
      맞는 값을 찾으면 **부팅 때마다 적용되게 고정**해야 한다(labwc autostart 또는
      `~/.config/wayfire.ini`). 터치스크린이면 **터치 좌표도 함께 도는지** 반드시 확인할 것 —
      화면만 돌고 터치는 안 도는 것이 흔한 함정이다.
      → 되는 방법을 찾으면 `deploy/pi/install-autostart.sh`에 넣고 런북 §3에 적는다.
- [ ] **🔴 마이크 입력(서버측 STT)** — 위 "새 1순위" 항목. 지금은 어르신이 로봇에게 말을
      걸 방법이 아예 없다. **다른 무엇보다 이것이 시연을 좌우한다.**
