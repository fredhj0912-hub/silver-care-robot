...
- [ ] **할당량이 돌아왔는지 확인** — 아무 대화나 한 번 보내 `source`가 `gemini`인지 본다
- [ ] **`VITE_VISION_ENABLED=false`로 호출을 줄인다** — 지금 EC2에 켜져 있어 카메라가
      **15초마다** Gemini Vision을 부른다(시간당 240건). 낙상 감지는 원래 YOLOv8 몫이고
      Vision은 표정 요약용이라 시연 필수가 아니다. 끄고 빌드 → `systemctl restart`
- [ ] **무료 등급으로 상시 운영이 되는가를 정한다** — 결제 활성화 / 호출 빈도 조정 /
      캐싱 중 무엇으로 갈지. **행사 전에 결론이 있어야 한다**
