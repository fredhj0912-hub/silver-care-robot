...
09-01·09-02에 두 번 앞에 섰다. 남은 것은 아래 순서대로다.

### 🔴 1순위 — Gemini API 할당량 (2026-09-02에 처음 부딪힌 블로커)

**이게 안 풀리면 아무것도 확인할 수 없다.** 받아쓰기는 물론 대화까지 mock으로 떨어진다
(`429 exceeded your current quota`). 대화·표정·받아쓰기·음성 합성이 **키 하나**를 공유한다.

**2026-09-02 오후에 실측한 것 — 문제는 "일시적 소진"이 아니라 무료 등급의 구조다:**

> `quotaId: GenerateRequestsPerDayPerProjectPerModel-FreeTier`, **`limit: 20`**.
> 무료 등급은 **모델 하나당 하루 20건**이다. 시연 한 번에 대화·받아쓰기·표정이 각각
> 호출되므로 **20건은 몇 분이면 사라진다.** 하루가 지나면 리셋되지만 그뿐이다.
> 할당량은 **모델별로 따로** 잡힌다 — 같은 시각에 `gemini-3.6-flash`는 429였고
> `gemini-3.5-flash`는 200이었다.

- [x] **할당량이 돌아왔는지 확인** — 하루가 지나 리셋됐다가 **몇 분 만에 다시 소진**됐다.
      "돌아왔다/막혔다"가 아니라 **하루 20건짜리 통**이라는 것이 답이다
- [x] **`VITE_VISION_ENABLED=false`로 호출을 줄인다** — EC2에서 껐고 재빌드·재시작까지
      마쳤다(번들에 `enabled:false`로 박힌 것 확인). 15초마다 돌던 Vision 호출이 사라졌다
- [ ] **무료 등급으로 상시 운영이 되는가를 정한다** → **하루 20건으로는 불가능하다.**
      남은 선택지는 사실상 **결제 활성화** 하나다. 호출 빈도 조정·캐싱은 20건 앞에서는
      의미가 없다. **행사 전에 결제를 켜 둘 것.**
- [x] **할당량이 소진돼도 대체 모델로 안 넘어가던 것을 고쳤다** (2026-09-02 발견·수정).
      `services/gemini.js`의 `withRetry`가 `QUOTA_EXHAUSTED`를 만나면 `throw`로 **체인
      전체를 빠져나가** `GEMINI_FALLBACK_MODEL`을 시도조차 하지 않았다. *같은 모델을
      재시도하지 않는 것*은 맞지만 *다음 모델로 넘어가지 않는 것*까지 딸려간 것이다.
      이제 재시도만 접고 모델은 넘긴다 — 무료 등급에서 쓸 수 있는 통이 20 → 40이 된다
      (그래도 결제를 대신하지는 못한다). **EC2 배포까지 마쳤고 실동작을 확인했다** —
      기본 모델이 429일 때 오류 메시지가 대체 모델 이름을 가리키는 것으로 전환을 확인했다

### 🔴 2순위 — 파이 앞에서만 되는 것 (위에서부터 순서대로)

하드웨어가 전부 붙어 있어야 하므로 **파이 앞에 있는 시간에 몰아서** 한다.
현장 절차는 `docs/pi-runbook.md`를 위에서부터 그대로 따라간다.

- [ ] **🔴 서버측 STT 실측** (런북 §6-3) — 09-02에 배관까지 확인했다. 남은 것:
      - [ ] **VAD 임계값이 reSpeaker에 맞는가** — `frontend/src/lib/vad.js`의
            `startThreshold=0.02`. 안 잡히면 낮추고, 잡음에 계속 반응하면 올린다.
            **이 값 하나가 성패를 가른다**
      - [ ] 로봇이 **자기 목소리에 반응하지 않는지** (스피커 볼륨을 올려 놓고)
      - [ ] **왕복 지연 측정** — 말을 마치고 대답까지. 너무 길면 `vad.js`의 `silenceMs`부터 줄인다
- [ ] **말풍선 가독성** — 09-02에 1rem→1.55rem으로 키웠다. 800×480 실물에서 읽히는지 눈으로 본다
- [ ] **원격조종 왕복 지연 측정** (런북 §7-2) — 이제 **`--dry-run` 으로 모터 없이 잴 수 있다**:
      `python3 deploy/pi/drivetrain/drivetrain.py --api "$KIOSK_URL" --key "$ROBOT_API_KEY" --dry-run`
      → 폰에서 화살표를 누르고 있으면 `drive()`, 떼면 `stop()` 이 찍힌다.
      **누른 순간→`drive`, 뗀 순간→`stop`** 을 3~5회. **정지 쪽 숫자가 더 중요하다** —
      늦게 서는 것은 위험이고 늦게 출발하는 것은 답답함일 뿐이다.
      이 값이 `HEARTBEAT_MS`/`STALE_MS`/`POLL_MS`/속도 상한(지금 전부 잠정값)의 유일한 근거다.
      09-01에는 인디케이터 버그를 고치느라, 09-02에는 STT에 매달리느라 못 쟀다
- [ ] **§8 낙상 알림 배관** — 카메라 칩 → `/guardian/live` → 푸시 재구독 →
      EC2에서 `npm run mock-detector` → 폰 푸시 → 파이 비상 모드 → 해제.
      **§7보다 나중에** — 응급 상태에서는 원격조종이 `423`으로 잠긴다
- [ ] **§9 구동부 실사** — `pinctrl get` / `i2cdetect -y 1`로 무엇이 어떻게 붙어 있는지 본다.
      그리고 **예전에 로봇을 움직이던 제어 코드를 찾는다**(다른 레포? SD카드? 팀원?).
      찾으면 `deploy/pi/drivetrain/motors.py` 하나만 채우면 된다 — 나머지는 09-02에 만들어 뒀다.
...
- [ ] **미디어 권한 대화상자** — 재부팅 후 또 물어보는지 확인. 안 물어보면 프로필에
      저장된 것이라 큰 문제가 아니다. 또 물어보면 Chromium 정책 파일이 필요하다
...
- [ ] **구동부 소비자 프로세스** — **골격은 09-02에 만들었다**(`deploy/pi/drivetrain/`).
      폴링·만료·정지·종료 처리가 다 들어 있고 `--dry-run` 으로 모터 없이 지연을 잴 수 있다.
      **남은 것은 `motors.py` 하나** — 예전 제어 코드를 찾아 `drive()`/`stop()` 을 채우는 일.
