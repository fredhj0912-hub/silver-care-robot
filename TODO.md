...
- [ ] **원격조종 왕복 지연 측정** (런북 §7) — 화살표를 누르고 파이 화면에 표시가 뜨기까지 3~5회.
      이 숫자가 **데드맨 500ms vs 폴링 2.5초** 정책의 유일한 근거다(B 트랙).
...
- [ ] **원격조종 네트워크 지연/단절 정책** (Phase 7 잔여) — 500ms 데드맨보다 명령 전송이
      늦어지면 로봇이 끊겨 이동하거나 정지 요청이 유실될 수 있다. 시뮬레이션에선 안
      드러나고 실물에서만 문제가 된다. **부분 해결(08-31)**: 조회에 `?maxAgeMs=`를 걸어
      낡은 이동 명령을 빼도록 했다. **2순위의 지연 측정값을 기다리고 있다**
