...
### 🔴 배포 전 점검에서 드러난 것 (2026-09-02, EC2 실측)

파이에 가기 전에 TTS로 만든 음성을 그대로 STT에 넣어 왕복시켰다. **받아쓰기 자체는
정확했다**("아이고 허리야 못 일어나겠어" → "아이코 허리야 못 일어나겠어"). 형식 문제는 없다.
문제는 다른 데 있었다.

**실측 지연** — 같은 3.6초짜리 오디오를 4번 보냈다: **3.0초 / 3.4초 / 20.3초 / 52.3초.**
느린 쪽은 Gemini의 `503 (high demand)` 재시도 체인이었다.

- [x] **실패가 침묵처럼 보이던 것** — 503으로 실패해도 `200 + 빈 text`를 돌려줬다.
      프론트는 그것을 침묵으로 취급하므로 **어르신이 말을 걸었는데 아무 일도 안 일어나고
      화면에 진단도 안 떴다.** 이제 되돌릴 수 없는 것은 503, 일시적인 것은 502로 나눈다
- [x] **50초를 잡아먹던 재시도** — 받아쓰기에 시한(`STT_TIMEOUT_MS`, 기본 12초)을 두고,
      같은 모델 재시도는 하지 않는다(`retries: 0`). 늦게 온 받아쓰기는 쓸모가 없다 —
      어르신은 이미 돌아섰고 그 사이에 한 다른 말과 뒤섞인다. 다음 발화에서 다시 시도된다
- [x] **파이에서 `chmod +x`가 다음 `git pull`을 막던 것** — 실행 권한을 git에 넣고,
      파이에서는 `git config core.fileMode false`로 모드 추적 자체를 껐다.
      권한을 커밋하는 것만으로는 부족했다 — **같은 일을 두 번 겪고 나서야** 원인이
      "권한이 없다"가 아니라 "git이 권한을 본다"라는 것을 알았다
- [x] **`preflight.sh`가 멀쩡한 스피커를 마이크라고 오탐하던 것** — `wpctl status`의
      구획 사이가 빈 줄이 아니라 `│` 라서 `Sinks:`에서 안 끊기고 `Sources:`의 기본
      장치까지 함께 집혔다. 09-01의 진짜 문제와 똑같은 문구가 떠서 더 헷갈렸다.
      소리 확인도 `speaker-test`(ALSA default → 파이에서는 HDMI)에서 `pw-play`로 바꿨다
- [x] **런북이 `git checkout main`을 시키던 것** — `deploy/pi/`는 그 브랜치에 없다.
      09-02에 이 문서대로 하다가 실제로 막혔다

- [ ] 🔴 **TTS도 503이면 브라우저 TTS로 폴백한다** — 그런데 **파이에서 브라우저 TTS는
      무음**이다(09-01 실측). 즉 Gemini가 붐비면 **로봇이 조용히 벙어리가 된다.**
      화면에 글은 뜨므로 완전한 실패는 아니지만, 소리로만 듣는 어르신에게는 실패다.
      선택지: (a) `npm run prewarm-tts`로 자주 쓰는 문장 캐싱 — 보호자가 임의로 친
      문장에는 안 통한다, (b) TTS에도 재시도 추가, (c) `TTS_PROVIDER=cloud` 전환
      (콘솔에서 API 활성화 1회 필요). **시연 전에 정해야 한다.**

### 🔴 파이 1차 실측 결과 (2026-09-02 저녁)

**배관은 통했다.** 파이 마이크 → VAD → WAV → EC2 → Gemini까지 실제로 도달했다.
화면 회전도 되고, 스피커·마이크 기본 장치도 맞게 잡혔다.

**막힌 곳은 Gemini API 할당량이었다** (`429 exceeded your current quota`).
그리고 그 뒤에 우리 결함 두 개가 숨어 있었다.

- [x] **포기해도 마이크가 계속 돌았다** — 브라우저 STT는 오류가 나면 세션이 스스로
      끝나서 `shouldListenRef` 플래그만으로 충분했다. **서버 STT의 세션은 스스로 끝나지
      않는다.** `disableStt()`가 인식기를 실제로 멈추지 않아, 포기한 뒤에도 발화마다
      업로드를 계속했다 — **한 시간에 100건.** 할당량이 이렇게 탔다
- [x] **할당량 소진(429)을 재시도했다** — 429는 두 가지다. 분당 한도(잠시 후 풀린다)와
      할당량 소진(오늘은 안 풀린다). 후자를 재시도하면 시간만 버리는 게 아니라
      **남은 할당량을 더 태운다**
- [ ] 🔴 **할당량이 돌아오면 다시 시험한다.** 그때까지는 STT를 확인할 방법이 없다
- [ ] **`VITE_VISION_ENABLED=true`가 EC2에 켜져 있다** — 카메라가 15초마다 Gemini Vision을
      부른다(시간당 240건). STT를 시험하는 동안에는 꺼 두는 것이 맞다. 낙상 감지는
      원래 YOLOv8 몫이고 Vision은 표정 요약용이라 시연에 필수가 아니다
- [ ] **무료 할당량으로 상시 운영이 되는가** — 이번에 처음 부딪혔다. 대화 + 표정 + 받아쓰기
      + 음성 합성이 전부 같은 키를 쓴다. 행사 전에 결제 활성화나 호출 빈도 조정을 정해야 한다
...
- [ ] **🔴 서버측 STT 실측** — 09-02에 구현 완료. **VAD 임계값이 reSpeaker에 맞는지가 핵심**
      ...
      EC2에서 왕복은 이미 확인했다(위 절) — 남은 것은 **파이 마이크가 어르신 목소리를
      잡는가**와 **Gemini 503이 얼마나 자주 나는가**다
