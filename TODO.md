...
**2026-08-31 기준 당장 일정이 없다.** 그래서 순서는 "시연 임박"이 아니라 **기술 부채를
줄이는 쪽**으로 잡았다. 급한 일정이 생기면 이 순서를 다시 짤 것.
...
### ⚠️ 어느 트랙부터 할지 — 먼저 판단할 것

하드웨어(라즈베리파이 5·카메라)가 **아직 없다**(08-31 기준). 그래서 트랙이 둘이다:

> - 사용자가 **"라즈베리파이 있어"** 같은 말을 하면 → **B 트랙을 1순위로** 올린다.
> - 아무 말이 없으면 → **A 트랙**을 위에서부터 순서대로 진행한다.

B 트랙은 실물 없이는 진행 자체가 불가능하므로 이 분기가 필요하다. 확실하지 않으면 물어볼 것.

### A 트랙 — 소프트웨어 (기본값, 전부 로컬에서 가능)

- [ ] **RDS 마스터 암호 교체** — 지금 값은 사전 공격에 즉시 뚫리는 수준인데 인스턴스는
      퍼블릭 액세스 상태다. **팀원 앱 설정도 함께 바꿔야 하므로 조율이 필요**해 제일 먼저
      꺼내 둔다. 바꾼 뒤 로컬·EC2 양쪽 `.env`와 팀원 쪽을 모두 갱신하고
      `npm run verify-rds`로 확인한다. (현재 값은 이 공개 레포에 적지 않는다.)
